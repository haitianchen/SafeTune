...
class MySQLEnv(DBEnv):
    ...
    def get_knobs_value_od(self,knobs, knobs_detail):
        value = []
        for k in knobs:
            type_ = knobs_detail[k]['type']
            if type_ == 'enum':
                v = knobs_detail[k]['enum_values'].index(knobs[k])
            else:
                v = knobs[k]
            value.append(v)
    ...
    def apply_knobs(self, knobs):
      ...
      #  knobs.
        ...
        try:
            ...
            # sql3 = 'set global general_log = 0 ;'
            # sql4 = 'set global general_log_file = "/home/ht/knobsTuning/KnobsTuningEA/scripts/sql_logs/sql_'+str(self.step_count)+'.log";'
            # r3 = db_conn.execute(sql3)
            # r4 = db_conn.execute(sql4)
            sql5 = 'show variables like "general%";'
            r5 = db_conn.fetch_results(sql5)
            print(r5)
    ...
    def _kill_mysqld(self):
        ...
        print('mysql is shut down')
        logger.info('mysql is shut down')

    def _start_mysqld(self):
        print('924',self.mycnf)
        print('924', self.mysqld)
        ...
        while True:
            try:
              ...
              #  print(dbc)
                ...
                pass
            print(count, self.host, self.port, self.user, self.passwd, self.dbname, self.sock)
            print('retry')
            time.sleep(1)
            count = count + 1
            if count > 600:
    ...
    def reinitdb_magic(self):
      ...
      #  print(src_data_path,dst_data_path)
    ...
    def step_SMAC(self, knobs, seed):
        ...
        record = "{}|{}|{}\n".format(knobs_display, list(external_metrics),list(internal_metrics))
        f.write(record)
        f.close()
    ...
    def step_openbox(self, config):
        ...
        #todo: recall ML model to get metrics , need to have a sample factory
    ...
    def step_openbox_od(self, config):
        f = open(self.lhs_log, 'a')
        knobs = config.get_dictionary().copy()
        #print(type(knobs),knobs)
        for k in knobs.keys():
            if self.knobs_detail[k]['type'] == 'integer' and  self.knobs_detail[k]['max'] > sys.maxsize:
                knobs[k] = knobs[k] * 1000
        metrics, internal_metrics, resource = self.step_GP_od(knobs)

        # record = "{}|{}\n".format(knobs, list(internal_metrics))
        # f.write(record)
        if metrics[0] < -50:
            format_str = '{}|tps_{}|[]|65d_odd\n'
            res = format_str.format(knobs, -np.random.random())
            f.write(res)
        elif   metrics[0] < 0 :
            format_str = '{}|tps_{}|[]|65d\n'
    ...
    def getMetric(self,logs, metric="tps_"):
        knobs = []
        metric_data = []
        for i in logs:
            temp = i.split('|')
            knobs.append(eval(temp[0]))
            for j in temp:
                #  print(j)
                if metric in j:
                    metric_data.append(float(j.split('_')[1]))
        return knobs, metric_data
    def step_GP_od(self, knobs, best_action_applied=False):

        with open(self.lhs_log, 'r') as f:
            logs = f.readlines()
        useOdData=False
        if 'ood' in logs[-1]:
            useOdData=True
        ...
        print(ccount)
        if useOdData:
            knobs_his, metric_data = self.getMetric(logs, 'tps_')
        else:
            pure_logs =[]
            for i in logs:
                if 'ood' in i :
                    ...
                    pure_logs.append(i)
            knobs_his, metric_data = self.getMetric(pure_logs, 'tps_')
        odx= []
        ody = []
        for k in range(len(knobs_his)):
                odx.append(self.get_knobs_value_od(knobs_his[k], self.knobs_detail))
                ody.append(metric_data[k])
        ...
        self.reinit_interval = self.reinit_interval + 1
        knobs_v = self.get_knobs_value_od(knobs, self.knobs_detail)
     ...
     #   return [-1, 300, -1], np.array([0] * 65), 0
        ...
        if ccount > 10:
            print('od start ')
            if self.is_outPoint(knobs_v,odx,ody) or self.vote_by_clf(knobs_v,odx,ody):
                return [-100, 1000, -100], np.array([0] * 65), 0
    ...
    def vote_by_clf(self,knobs,odx ,ody):
        xx = []
        yy = []
        for i in range(len(odx)):
            for j in range(i + 1, len(odx)):
                if ody[i] > odx[j]:
                    xx.append(odx[i] + odx[j])
                    yy.append(0)
                else:
                    xx.append(odx[i] + odx[j])
                    yy.append(1)
        ...
        safe_th = max(ody) * 0.70
        clf.fit(xx, yy)
        wi = []
        for i in range(len(odx)):
            if ody[i] < safe_th:
                wi.append(knobs,odx[i])
        res = clf.predict(wi)
        count_0 = []
        count_1 = []
        for i in res:
    ...
    def is_outPoint(self,knobs_v,odx,ody):
        ...
        if len(odx) > 10 :
            ...
            nei = 15
        detector_list = [LOF(n_neighbors=nei), KNN(n_neighbors=nei),
                         COPOD(), IForest(n_estimators=100), QMCD()
                         ]

        xgbod = XGBOD(estimator_list=detector_list)
        xg_label = []
        xg_x = odx[:]
        safe_th = max(ody) * 0.70
        ...
        xgbod.fit(xg_x, xg_label)
        y = xgbod.predict([knobs_v])
        print('label : ', y )
        #time.sleep(10)
        if y[0] ==1 :
            print('outline point')
